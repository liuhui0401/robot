    ...
    */
    ...
    //==============================================================================
        ...
        //3.计算激光点在激光雷达坐标系下的位置，并根据参数inputparams_0.front()->isReverse判断是否将lx取相反数。
            ...
            lx = dis * cos(angle);
            ly = dis * sin(angle);
            if(inputparams_0.front()->isReverse) lx = -lx;
        ...
        //4.进行 激光雷达坐标系->机器人坐标系 变换
            ...
            rx = inputparams_0.front()->xL - dis * cos(angle + laser_aL_rad);
            ry = inputparams_0.front()->yL + dis * sin(angle + laser_aL_rad);

        //5.进行 机器人坐标系->全局坐标系 变换
            //机器人航向角ori定义见课件，对应变量为inputdata_1.front()->ori
            double orien = inputdata_1.front()->ori;
            gx = inputdata_1.front()->x + cos(orien) * ry - sin(orien) * rx;
            gy = inputdata_1.front()->y + cos(orien) * rx + sin(orien) * ry;
        ...
        //8.根据OGM地图生成方法制图。
            ...
            for (std::vector<Location>::iterator c=locationVec.begin();c!=locationVec.end();c++){
                if((*c).x >= 0 && (*c).x < params->mapWidth && (*c).y >= 0 && (*c).y < params->mapHeight){
                    int xx = (*c).x, yy = (*c).y;
                    if(xx == mapx && yy == mapy){
                        vars->map[yy][xx] += params->logodd_occu;
                    }
                    else vars->map[yy][xx] += params->logodd_free;
                    if(vars->map[yy][xx] < lowthres) vars->map[yy][xx] = lowthres;
                    if(vars->map[yy][xx] > upthres) vars->map[yy][xx] = upthres;
