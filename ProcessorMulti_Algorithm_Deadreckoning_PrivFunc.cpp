...
/**************************************************填写以下部分***************************************************************/
    ...
    if (deltaPluse < -pluseMax/2)
        deltaPluse += pluseMax;
    double distance = (double)deltaPluse * disPerPluse;
    // --------------------处理编码器数据溢出,并将单位转换为m-----------------------------------------------


    // --------------------计算小车当前时刻的位置，并更新last数据--------------------
    double averageOri  = (currentOri + vars->lastori) / 2;
	vars->lastx += distance * cos(averageOri);
	vars->lasty += distance * sin(averageOri);
    ...
    // ----------------------填充outputdata---------------------------------------------
    outputdata->orientation = currentOri + vars->PI / 2;
